...
## Эндпоинт: `dialogMarkAs`

### Описание
Отмечает определенное событие для диалога (например, открытие или изучение диалога).

### Параметры запроса
| Параметр  | Тип    | Обязательный | Описание                    |
|-----------|--------|--------------|----------------------------|
| dialogId  | string | Да           | Идентификатор диалога      |
| event     | string | Да           | Тип события. Возможные значения: "opened", "learned" |

### Формат ответа
Успешный ответ не содержит данных.
...
| Ошибка                  | Описание                                     |
|------------------------|----------------------------------------------|
| Dialog ID is required  | Не указан идентификатор диалога             |
| Unsupported: {event}   | Указан неподдерживаемый тип события         |
| Dialog not found       | Диалог с указанным ID не найден             |
